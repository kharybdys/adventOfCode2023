...
from puzzle2022_11.monkey import Monkey
from puzzle2022_11.parser import parse_puzzle_lines


def expected_output(example: bool) -> list[str]:
    example_postfix = "_example" if example else ""
    with open(Path(__file__).parent / f"expected_output{example_postfix}.txt", "rt") as file:
...
def solve_a(puzzle_input: list[str]) -> None:
    print(puzzle_input)
    logger = LogStreamer()
    monkeys: dict[int, Monkey] = parse_puzzle_lines(puzzle_input, logger)
